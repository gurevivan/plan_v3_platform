...
# Имя контейнера базы зависит от того, как разворачивали: `plan-db` ставит
# `setup.sh`, `plan-pg` остался от ранней ручной установки. Ищем то, что есть,
# вместо того чтобы держать два почти одинаковых скрипта.
CONT=""
